    ...
    public void migrate(DatabaseConnector connector, Connection connection, String tablePrefix) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            if (connector instanceof SQLiteConnector) {
                ...
                statement.addBatch("ALTER TABLE " + tablePrefix + "tag_data DROP COLUMN `icon`");
                statement.addBatch("ALTER TABLE " + tablePrefix + "tag_data MODIFY COLUMN `icon` BLOB NULL");
                statement.addBatch("ALTER TABLE " + tablePrefix + "tag_data MODIFY COLUMN `category` TEXT NULL");
