...
public class CreateCommand extends RoseCommand {
    ...
    @RoseExecutable
    public void execute(CommandContext context, String name, GreedyString tagMessage) {
        ...
        Tag tag = new Tag(name.toLowerCase(), name, tagMessage.get());
        tag.setDescription(Collections.singletonList("None"));

        if (manager.saveTag(tag)) {
            locale.sendMessage(sender, "command-create-created", StringPlaceholders.single("tag", tag.getTag()));
        }
    }
